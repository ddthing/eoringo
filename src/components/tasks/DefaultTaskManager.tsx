...
export const DefaultTaskManager=({tasks,query="",status="enabled",resetFilter="all",now=new Date()}:Props)=>{
  ...
  const matchingTasks = useMemo(
    () => tasks ?? defaultTaskTemplates.filter(
      (task) => matchesManagedTask(task, query, resetFilter)
        && (status === "all" || (status === "hidden") === disabledSet.has(task.id)),
    ),
    [disabledSet, query, resetFilter, status, tasks],
  );
